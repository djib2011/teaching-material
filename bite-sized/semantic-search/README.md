...
1. Install dependencies:
	- recommended way is through uv
	```bash
	cd bite-sized/semantic-search  # go to the project root
	uv venv  # create virtual environment via uv
	uv venv synv  # install dependencies
	```
	- or you can do it via a regular python virtual environment
		- requirements can be found in the `requirements.txt` file.
		- create the venv and install dependenies
		```bash
		cd bite-sized/semantic-search  # go to the project root
		...
		```
	- To run notebooks with this virtual environment
		```python
		source .venv/bin/activate  # or semantic-search-env/bin/activate if you went with the second option
